...
func (v *vaultPKI) ReconcilePKI(ctx context.Context, logger logr.Logger, scheme *runtime.Scheme, externalHostnames, externalIps []string) (err error) {
	...
	brokerCert, err := v.reconcileBrokerCert(ctx, vault, externalHostnames, externalIps)
	if err != nil {
		return
	}
...
func (v *vaultPKI) reconcileBrokerCert(ctx context.Context, vault *vaultapi.Client, externalHostnames, externalIps []string) (*pkicommon.UserCertificate, error) {
	return v.reconcileStartupUser(ctx, vault, pkicommon.BrokerUserForCluster(v.cluster, externalHostnames, externalIps))
}
